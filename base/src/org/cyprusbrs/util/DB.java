...
import org.cyprusbrs.db.ProxyFactory;
import org.cyprusbrs.model.MAcctSchema;
import org.cyprusbrs.model.MLanguage;
...
import org.cyprusbrs.process.SequenceCheck;
...
public final class DB
{
	...
	public static boolean afterMigration (Properties ctx)
	{
		...
		try
		{
			Class<?> clazz = Class.forName("org.cyprusbrs.MigrateData");
			clazz.newInstance();
		}
		catch (Exception e)
		{
	...
	{
		//	Get Property File
		String envName = Ini.getCyprusHome();
		if (envName == null)
			return;
		envName += File.separator + "CyprusEnv.properties";
	 ...
	 *  @return Cyprus Database Driver
	 */
	public static CyprusDatabase getDatabase()
	{
		if (s_cc != null)
	 ...
	 *  @return Cyprus Database Driver
	 */
	public static CyprusDatabase getDatabase(String URL)
	{
...
//    Check Version
        ...
        if (Cyprus.DB_VERSION.equals(version))
            return true;

        String AD_Message = "DatabaseVersionError";
        String title = org.cyprusbrs.Cyprus.getName() + " " +  Msg.getMsg(ctx, AD_Message, true);
        //  Code assumes Database version {0}, but Database has Version {1}.
        String msg = Msg.getMsg(ctx, AD_Message);   //  complete message
        msg = MessageFormat.format(msg, new Object[] {Cyprus.DB_VERSION, version});
	...
	{
//    Check Build
        String buildClient = Cyprus.getImplementationVersion();
	 ...
	 *  @see org.cyprusbrs.util.DisplayType
	 *  @see org.cyprusbrs.util.Env
	 *
	 *   */
