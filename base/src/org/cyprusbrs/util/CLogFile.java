...
public class CLogFile extends Handler
{
	 ...
	 *	@param cyprusHome cyprus home
	 *	@param isClient client
	 *	@return file logger
	 */
	public static CLogFile get (boolean create, String cyprusHome, boolean isClient)
	{
		if (s_logFile == null && create)
			s_logFile = new CLogFile(cyprusHome, true, isClient);
		return s_logFile;
	}	//	get
	 ...
	 *	@param isClient client
	 */
	public CLogFile (String cyprusHome, boolean createLogDir, boolean isClient)
	{
		if (s_logFile == null)
			s_logFile = this;
		else
		...
		if (cyprusHome != null && cyprusHome.length() > 0)
			m_cyprusHome = cyprusHome;
		else
			m_cyprusHome = Ini.findCyprusHome();
		initialize(m_cyprusHome, createLogDir, isClient);
	}	//	CLogFile

	/**	Cyprus Home			*/
	private String		m_cyprusHome = null;
	/**	Printed header			*/
	...
	/**
	 * 	Initialize
	 * 	@param cyprusHome log file base directory name
	 * 	@param createLogDir create log directory
	 * 	@param isClient client
	 */
	private void initialize(String cyprusHome, boolean createLogDir, boolean isClient)
	{
	//	System.out.println("CLogFile.initialize");
		...
		m_doneHeader = false;
		//	New File Name
		if (!createFile(cyprusHome, createLogDir, isClient))
			return;
		//	New Writer
		try
		{
	...
	private boolean createFile(String baseDirName, boolean createLogDir, boolean isClient)
	{
		...
		try
		{
			...
			//	Test/Create CyprusHome/log/file
			if (fileName != null)
			{
			...
			if (m_file == null)	{	//	Fallback create temp file
				m_fileNameDate = getFileNameDate(System.currentTimeMillis());
				m_file = File.createTempFile("cyprus"+m_fileNameDate + "_", ".log");
			}
		}
		catch (Exception ex)
		{
	...
	{
		initialize(m_cyprusHome, true, Ini.isClient());
	}	//	rotateLog
