...
import org.cyprusbrs.db.CConnectionDialog;
...
public class Trace
{
	...
	public static String[] getCallerClasses (Throwable caller, int maxNestLevel)
	{
		...
		//	System.out.println(list.size() + ": " + className);
			if (!(className.startsWith("org.cyprusbrs.util.Trace")
	...
	//	t.printStackTrace();
		...
		for (int i = 1; i < elements.length; i++)
		{
			if (elements[i].getClassName().indexOf("util.Trace") != -1)
				continue;
			if (!adempiereOnly
				|| (adempiereOnly && elements[i].getClassName().startsWith("org.cyprusbrs"))
				)
			{
