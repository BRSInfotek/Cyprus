...
public class SecureEngine
{
	/**
	 * 	Initialize Security
	 *	@param ctx context with CYPRUS_SECURE class name
	 */
	public static void init (Properties ctx)
	{
		if (s_engine == null)
		{
			String className = ctx.getProperty(SecureInterface.CYPRUS_SECURE);
	 ...
	 * 	@param className class name if null defaults to org.cyprusbrs.util.Secure 
	 */
	private SecureEngine (String className)
	{
		String realClass = className;
		if (realClass == null || realClass.length() == 0)
			realClass = SecureInterface.CYPRUS_SECURE_DEFAULT;
		...
		if (implementation == null)
		{
			String msg = "Could not initialize: " + realClass + " - " + cause.toString()
				+ "\nCheck start script parameter CYPRUS_SECURE"; 
			log.severe(msg);
			System.err.println(msg);
			System.exit(10);
		}
