...
import org.cyprusbrs.model.MCash;
import org.cyprusbrs.model.MCashBook;
import org.cyprusbrs.model.MCashLine;
...
//		log.fine("Lines=" + p_lines.length);
		...
		/// Code updated by Anshul @20220128
		
		//	Contained Objects
		p_lines = loadLines(cash, cb);
		
		m_taxes = UtilTax.loadTaxes(get_ID(),UtilTax.CASHJOURNAL);
		p_lines = loadLines(cash,cb);
	...
	private DocLine[] loadLines(MCash cash)
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			//	Set Date Acct
			if (i == 0)
				setDateAcct(cash.getDateAcct());
			MPeriod period = MPeriod.get(getCtx(), cash.getDateAcct(), cash.getAD_Org_ID());
			if (period != null && period.isOpen(DOCTYPE_CashJournal, cash.getDateAcct()))
			...
			{  
				...
				//  Charge          DR          (Charge)
						...
						for(DocTax j:m_taxes) /// Need to remove this for loop... Single time run
						{
							...
							if (!j.isSalesTax() && line.getTaxAmount() !=null && line.getTaxAmount().signum() != 0) // Parent Tax
							{
								fl = fact.createLine(line,
										...
										line.getC_Currency_ID(), null,line.getTaxAmount().negate());
								if (fl != null)
							...
							else if(line.getTaxAmount() !=null && line.getTaxAmount().signum() != 0)// Child Tax
							{
								...
								for(DocTax childTax:m_sum_taxes)
								{
									fl = fact.createLine(line,
											...
											line.getC_Currency_ID(), null,childTax.getAmount().negate());
									if (fl != null)
					...
					//	ACCTTYPE_TaxDue
						...
						assetAmt = Env.ZERO;
						assetAmt = assetAmt.subtract(line.getAmount().negate().add(totalTaxAmount));
					//	assetAmt = assetAmt.subtract(line.getAmount().negate());
					//	assetAmt = line.getAmount().negate().add(totalTaxAmount);

//						fl = fact.createLine(line,
//								line.getChargeAccount(as, line.getAmount().negate()),
//								line.getC_Currency_ID(), null, line.getAmount().negate().add(totalTaxAmount));
					...
					for(DocTax j:m_taxes)
					{
						...
						if (!j.isSalesTax() && line.getTaxAmount() !=null && line.getTaxAmount().signum() != 0) // Parent Tax
						{
							fl = fact.createLine(line,
									...
									line.getC_Currency_ID(), line.getTaxAmount(),null);
							if (fl != null)
								fl.setC_Tax_ID(j.getC_Tax_ID());
						...
						else if(line.getTaxAmount() !=null && line.getTaxAmount().signum() != 0) // Child Tax
						{
							...
							for(DocTax childTax:m_sum_taxes)
							{
								...
								fl = fact.createLine(line,
										...
										line.getC_Currency_ID(), childTax.getAmount(),null);
								if (fl != null)
									fl.setC_Tax_ID(childTax.getC_Tax_ID());
						...
						System.out.println(" totalTaxAmount "+totalTaxAmount);
						assetAmt = Env.ZERO;
						assetAmt = assetAmt.add(line.getAmount().subtract(totalTaxAmount));
					//	assetAmt = line.getAmount().subtract(totalTaxAmount);

//						fl = fact.createLine(line,
//								line.getChargeAccount(as, line.getAmount()),
//								line.getC_Currency_ID(), (line.getAmount().subtract(totalTaxAmount)), null);
					...
					/// End of code by Anshul @20220128================================================================
				}	//  lines
