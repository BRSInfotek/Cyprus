 ...
 *****************************************************************************/
package org.cyprusbrs.sla;

import java.sql.Timestamp;
import java.util.logging.Level;

import org.cyprusbrs.model.MSLACriteria;
import org.cyprusbrs.model.MSLAGoal;
import org.cyprusbrs.model.MSLAMeasure;
...
public class SLAMeasureProcess extends SvrProcess
{
	...
	protected String doIt () throws Exception
	{
		...
		if (measure.get_ID() == 0)
			throw new CyprusUserError("@PA_SLA_Measure_ID@ " + p_PA_SLA_Measure_ID);
		
		MSLAGoal goal = new MSLAGoal(getCtx(), measure.getPA_SLA_Goal_ID(), get_TrxName());
		if (goal.get_ID() == 0)
			throw new CyprusUserError("@PA_SLA_Goal_ID@ " + measure.getPA_SLA_Goal_ID());

		MSLACriteria criteria = MSLACriteria.get(getCtx(), goal.getPA_SLA_Criteria_ID(), get_TrxName());
		if (criteria.get_ID() == 0)
			throw new CyprusUserError("@PA_SLA_Criteria_ID@ " + goal.getPA_SLA_Criteria_ID());
