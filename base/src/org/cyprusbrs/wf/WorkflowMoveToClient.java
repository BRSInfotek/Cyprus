...
public class WorkflowMoveToClient extends SvrProcess
{
	...
	protected String doIt () throws Exception
	{
		...
		if (no == -1)
			throw new CyprusSystemError ("Error updating Workflow");
		changes += no;
		...
		if (no == -1)
			throw new CyprusSystemError ("Error updating Workflow Node");
		changes += no;
		...
		no = DB.executeUpdate(sql, get_TrxName());
		if (no == -1)
			throw new CyprusSystemError ("Error updating Workflow Transition");
		changes += no;
		...
		no = DB.executeUpdate(sql, get_TrxName());
		if (no == -1)
			throw new CyprusSystemError ("Error updating Workflow Node Parameters");
		changes += no;
		...
		no = DB.executeUpdate(sql, get_TrxName());
		if (no == -1)
			throw new CyprusSystemError ("Error updating Workflow Transition Condition");
		changes += no;
