...
import org.cyprusbrs.model.MNote;
import org.cyprusbrs.model.MOrg;
import org.cyprusbrs.model.MOrgInfo;
import org.cyprusbrs.model.MPInstance;
import org.cyprusbrs.model.MPInstancePara;
		...
		if (obj instanceof Exception)
		{
			...
			while (ex != null)
			{
				...
				for (int i = 0; i < st.length; i++)
				{
					...
					if (i == 0 || ste.getClassName().startsWith("org.cyprusbrs") || ste.getClassName().startsWith("org.cyprus"))
		...
		else if (MWFNode.ACTION_UserChoice.equals(action))
		{
				...
				&& getPO(trx) instanceof DocAction)
			{
				...
				else	//	fixed Approver
				{
					...
					{
						throw new CyprusException("Support not implemented for "+resp);
					}
					else
					{
						throw new CyprusException("@NotSupported@ "+resp);
					}
					// end MZ
				}
				if (autoApproval
