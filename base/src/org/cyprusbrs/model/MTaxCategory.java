...
public class MTaxCategory extends X_C_TaxCategory
{
	...
	public MTax getDefaultTax()
	{
		...
		List<MTax> list = new Query(getCtx(), I_C_Tax.Table_Name, whereClause,  get_TrxName())
			...
			.list();
		if (list.size() == 0) {
			throw new CyprusException("NoDefaultTaxRate"); // Error - should be at least one default
		} else if (list.size() == 1) {
			m_tax = list.get(0);
		} else {
			throw new CyprusException("TooManyDefaults"); // Error - should only be one default
		}
