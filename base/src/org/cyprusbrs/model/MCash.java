...
import org.cyprus.util.UtilBankCashPayment;
import org.cyprus.util.UtilCashJournal;
...
public class MCash extends X_C_Cash implements DocAction
{
	...
	{
		/**
		 * @author Mukesh update the logic as per trello-Adempiere Costing @20201017
		 */
		if(newRecord)
		{
			boolean checkOldCashStatement=org.cyprus.util.UtilCashJournal.isAlreadyCashJournal(getAD_Client_ID(), getAD_Org_ID(), getC_CashBook_ID());
			log.info("Parameters : AD_Client_ID : "+getAD_Client_ID()+" AD_Org_ID : "+ getAD_Org_ID()+" C_CashBook_ID : "+getC_CashBook_ID());
			log.info("Result : "+checkOldCashStatement);
			if(checkOldCashStatement && isActive())
			{
				log.saveError(Msg.getMsg(getCtx(), "Cyprus_CashJournal_AlreadyExist"), "");
				return false;
			}
		
		}
		
		/** Check every time if Account Date save before current Date as per trello-Adempiere-Payment Module in "Cash Journal Update 20201225 (By Skype)" @20210107
		*/
		if(newRecord || is_ValueChanged(MCash.COLUMNNAME_DateAcct))
		{	
			Timestamp curren=new Timestamp(System.currentTimeMillis());
			...
			if(checkDate<0)
			{
				...
				log.saveError(Msg.getMsg(getCtx(), "Cyprus_CashJournal_BeforeCurrentDate"), "");
				//			log.saveError("You can not save Date before Current Date","You can not save Date before Current Date");
				return false;
			}
		}
		/// end of code
		...
		// update the Beginning Balance from Cash Book current balance as per discussed by Surya @20210313 by Mukesh
		if (! isProcessed() && getBeginningBalance().compareTo(Env.ZERO) == 0)
		{
			MCashBook ba = getCashBook();
			ba.load(get_TrxName());
			setBeginningBalance(ba.getCurrentBalance());
		}
		// End of code
		
		// in before save method 
		//Start implemented by Anil @20210629 as per excel file given by Mukesh
		if(is_ValueChanged(MCash.COLUMNNAME_StatementDate))
		{	
			if(UtilCashJournal.isSaveBackDatedRecords(getAD_Org_ID(),getAD_Client_ID(), getC_CashBook_ID()))
			{
				log.saveError(Msg.getMsg(getCtx(), "Cyprus_CashJournal_BeforeCurrentDate"), "");
				return false;				
			}
	 ...
	 *	updated by Mukesh @20210310
	 */
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		if (getDocumentNo() == null || getDocumentNo().isEmpty()) {
			setDocumentNo(String.valueOf(getC_Cash_ID()));
			saveEx();
		}
		return true;
	}
	...
	public String completeIt()
	{
		...
		//Start implemented by Anil @24/06/2021 as per excel file given by Mukesh		
		if(UtilBankCashPayment.isCompleteBackDatedRecordCashJournal(getStatementDate(),getAD_Client_ID(),getAD_Org_ID(),getC_CashBook_ID()))
		 {
			 m_processMsg =Msg.getMsg(getCtx(), "Cyprus_CashJournal_NotCompletedInBackDate");
			 return DocAction.STATUS_Invalid;
		 }
		//End implemented by Anil  @20210624 as per excel file given by @Mukesh 
		...
		// update the Current Balance in Cash Book from Ending balance in Cash Journal as per discussed by Surya @20210313 by Mukesh
		MCashBook ba = getCashBook();
		ba.load(get_TrxName());
		//BF 1933645
	...
	public boolean voidIt()
	{
		...
		//Start implemented by Anil @24/06/2021 as per excel file given by Mukesh
		 if(UtilBankCashPayment.isVoidBackDatedRecordCashJournal(getStatementDate(),getAD_Client_ID(),getAD_Org_ID(),getC_CashBook_ID()))
		 {
			 m_processMsg =Msg.getMsg(getCtx(), "Cyprus_CashJournal_NotVoidedInBackDate");
			return false;
		 }
		// Before Void
