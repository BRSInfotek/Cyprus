...
public class CalloutInventory extends CalloutEngine
{
	...
	// Implemented by Anshul@20220309 
	
	public String isInternalUse (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		if (value == null)
			return "";
		int C_DocType_ID = ((Integer)value).intValue();
		MDocType dt = MDocType.get(ctx, C_DocType_ID);
		dt.load(dt.get_TrxName());
		if(dt.isInternalUse() == true)
		{
		mTab.setValue("IsInternalUse", "Y");
		}
		else 
		{
			mTab.setValue("IsInternalUse", "N");
		}
		return "";
	}	//	isInternalUse
