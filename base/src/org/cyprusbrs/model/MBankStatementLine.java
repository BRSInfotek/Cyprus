 ...
 */
 public class MBankStatementLine extends X_C_BankStatementLine
 {
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		/** Check every time if Account Date save before current Date as per trello-Adempiere-Payment Module in "Cash Journal Update 20201225 (By Skype)" @20210113
		*/
		...
		// update Statement Amount, Transaction amt, Charge Amt and Interest Amount by Mukesh @20210113 
		if ((newRecord || is_ValueChanged(MBankStatementLine.COLUMNNAME_StmtAmt)))	//	allow different org for charge
			setStmtAmt(UtilPayment.getRoundingAmountFromStdPrecision(getStmtAmt(),getC_Currency_ID()));
		if((newRecord || is_ValueChanged(MBankStatementLine.COLUMNNAME_TrxAmt)))
			setTrxAmt(UtilPayment.getRoundingAmountFromStdPrecision(getTrxAmt(),getC_Currency_ID()));
		if((newRecord || is_ValueChanged(MBankStatementLine.COLUMNNAME_ChargeAmt)))
			setChargeAmt(UtilPayment.getRoundingAmountFromStdPrecision(getChargeAmt(),getC_Currency_ID()));
		if((newRecord || is_ValueChanged(MBankStatementLine.COLUMNNAME_InterestAmt)))
			setInterestAmt(UtilPayment.getRoundingAmountFromStdPrecision(getInterestAmt(),getC_Currency_ID()));
		// End of code
		
		// Calculate Tax Amount by Anshul @23062021 as per excel sheet given by Mukesh
		if(getChargeAmt().compareTo(Env.ZERO) != 0 || is_ValueChanged(MBankStatementLine.COLUMNNAME_C_Tax_ID))
		{
			BigDecimal taxamt = UtilTax.getTaxAmountFromStdPrecision(getChargeAmt(), getC_Tax_ID(), getC_Currency_ID());
			setTaxAmt (taxamt);
		}
		// End of the code
