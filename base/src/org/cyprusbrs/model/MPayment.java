	...
	implements DocAction, ProcessCall
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		// update payamt, discount amt, writeoff and over/under payment by Mukesh @20210113 
		if ((newRecord || is_ValueChanged(MPayment.COLUMNNAME_PayAmt)))	//	allow different org for charge
			setPayAmt(UtilPayment.getRoundingAmountFromStdPrecision(getPayAmt(),getC_Currency_ID()));
		if((newRecord || is_ValueChanged(MPayment.COLUMNNAME_DiscountAmt)))
			setDiscountAmt(UtilPayment.getRoundingAmountFromStdPrecision(getDiscountAmt(),getC_Currency_ID()));
		if((newRecord || is_ValueChanged(MPayment.COLUMNNAME_WriteOffAmt)))
			setWriteOffAmt(UtilPayment.getRoundingAmountFromStdPrecision(getWriteOffAmt(),getC_Currency_ID()));
		if((newRecord || is_ValueChanged(MPayment.COLUMNNAME_OverUnderAmt)))
			setOverUnderAmt(UtilPayment.getRoundingAmountFromStdPrecision(getOverUnderAmt(),getC_Currency_ID()));
		// End of code
		
		/// Updated the code by Anshul @20210625
	//	if(getPayAmt().compareTo(Env.ZERO) != 0 || is_ValueChanged(MPayment.COLUMNNAME_C_Tax_ID))
		
		if(String.valueOf(getC_Charge_ID())!=null || is_ValueChanged(MPayment.COLUMNNAME_C_Tax_ID))
		{	
			BigDecimal taxamt = UtilTax.getTaxAmountFromStdPrecision(getPayAmt(), getC_Tax_ID(), getC_Currency_ID());
			setTaxAmt (taxamt);
		}
		// End of the code
		return true;
...
*/
	...
	public String prepareIt()
	{
		...
		//  Not Required this check on complete button as Discussed by Surya : Updated by @Mukesh 20201025
...
*/
	...
	public boolean allocateIt()
	{
		...
		for (int i = 0; i < pAllocs.length; i++)
		{
			...
			if(pa.getC_Invoice_ID()>0)  /// only Invoice will create in Allocation Line window as discussed by Surya : @Mukesh 20201026 
			{
	...
	//		return;
		...
		for (int i = 0; i < allocations.length; i++)
		{
			...
			if (!allocations[i].processIt(DocAction.ACTION_Reverse_Correct))
				throw new CyprusException(allocations[i].getProcessMsg());
