...
import org.cyprusbrs.process.DocumentTypeVerify;
...
//	private String          m_orgName;
	...
	private void createAccountingRecord(String tableName) throws Exception
	{
		...
		//
		if (!acct.save()) {
			throw new CyprusUserError(CLogger.retrieveErrorString(table.getName() + " not created"));
		}
	}
	 ...
	 * @throws CyprusUserError 
	 */
	private Integer getAcct (String key) throws CyprusUserError
	{
		log.fine(key);
		//  Element
		...
		{
			throw new CyprusUserError("Account not defined: " + key);
		}
		...
		if (!vc.save())
		{
			throw new CyprusUserError("Not Saved - Key=" + key + ", C_ElementValue_ID=" + C_ElementValue_ID);
		}
		int C_ValidCombination_ID = vc.getC_ValidCombination_ID();
		if (C_ValidCombination_ID == 0)
		{
			throw new CyprusUserError("No account - Key=" + key + ", C_ElementValue_ID=" + C_ElementValue_ID);
		}
		return C_ValidCombination_ID;
	}   //  getAcct
