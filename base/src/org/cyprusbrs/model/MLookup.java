...
public final class MLookup extends Lookup implements Serializable
{
	 ...
	 *	@see org.cyprusbrs.model.Lookup#removeAllElements()
	 */
	@Override
		...
		public void run()
		{
			...
			if (!m_info.IsValidated)
			{
				...
				if (validation.length() == 0 && m_info.ValidationCode.length() > 0)
				{
					...
					if (Ini.isClient() == false || !Env.getWindow(m_info.WindowNo).getClass().getName().equals("org.cyprusbrs.print.Viewer")) {
						m_lookup.clear();
						return;
					}
				}
