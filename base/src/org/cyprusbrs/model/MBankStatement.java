...
import org.cyprus.util.UtilBankStatement;
...
public class MBankStatement extends X_C_BankStatement implements DocAction
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if(newRecord)
		{	
			boolean checkOldBankStatement=UtilBankStatement.isAlreadyBankStatement(getAD_Client_ID(), getAD_Org_ID(), getC_BankAccount_ID());
			log.info("Parameters : AD_Client_ID : "+getAD_Client_ID()+" AD_Org_ID : "+ getAD_Org_ID()+" C_BankAccount_ID : "+getC_BankAccount_ID());
			log.info("Result : "+checkOldBankStatement);
			if(checkOldBankStatement && isActive())
			{
				log.saveError(Msg.getMsg(getCtx(), "Cyprus_BankStatement_AlreadyExist"), "");
				return false;
			}
...
//		Date dateAcct=new Date(getStatementDate().getTime());
...
//			log.saveError(Msg.getMsg(getCtx(), "Cyprus_BankStatment_BeforeCurrentDate"), "");
////			log.saveError("You can not save Date before Current Date","You can not save Date before Current Date");
//			return false;
//		}
	}
	// Code updated by Anshul @20210629
	if(is_ValueChanged(MBankStatement.COLUMNNAME_StatementDate))
	{
		if(UtilBankStatement.isSaveBackDatedRecord(getAD_Org_ID(), getAD_Client_ID(), getC_BankAccount_ID()))
		{
			log.saveError(Msg.getMsg(getCtx(), "Cyprus_BankStatment_BeforeCurrentDate"), "");
			return false;
		}
		...
		if (getC_DocType_ID() <= 0)
		{
			...
			if (getC_DocType_ID() <= 0)
				throw new CyprusException("@C_DocType_ID@ @FillMandatory@");

		}
		/// end of code updated by Mukesh @20210310
	...
	public String completeIt()
	{
		...
		//Implemented By @Anshul on @20210625 as per excel file given by @Mukesh
		if(UtilBankCashPayment.isCompleteBackDatedRecordBankStatement(getStatementDate(),getAD_Client_ID(),getAD_Org_ID(),getC_BankAccount_ID()))
		 {
			 m_processMsg =Msg.getMsg(getCtx(), "Cyprus_BankStatement_NotCompletedInBackDate");
			return DocAction.STATUS_Invalid;
		 }
		/// End of the code by Anshul @20210625
	...
	public boolean voidIt()
	{
		...
		//Implemented By @Anshul on @20210624 as per excel file given by @Mukesh		
		if(UtilBankCashPayment.isVoidBackDatedRecordBankStatement(getStatementDate(),getAD_Client_ID(),getAD_Org_ID(),getC_BankAccount_ID()))
		{
			 m_processMsg =Msg.getMsg(getCtx(), "Cyprus_BankStatement_NotVoidedInBackDate");
			 return false;	
		}
		/// End of code by Anshul
