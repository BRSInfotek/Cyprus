...
//		MInventoryLine[] Inventorylines = getLines(false);
//		for (MInventoryLine line : Inventorylines)
//		{
//			if(UtilTransaction.isCompleteBackDatedRecordTransaction(getMovementDate(),UtilTransaction.PHYSICALINVENTORY,getAD_Client_ID(),getAD_Org_ID(),line.getM_Product_ID()))
//			{
...
//		
//		MInventoryLine[] linesInv = this.getLines(false);
//		Boolean isOpeningStock=this.isOpeningStock();
//		if(isOpeningStock)
...
//				  rs = pstmt.executeQuery();
//				 	if (rs.next()) {
//						int currentcostprice = rs.getInt(1);
//						int M_Product_ID = rs.getInt(2);
//						int C_AcctSchema_ID = rs.getInt(3);
//						int M_CostElement_ID = rs.getInt(4);
//						int AD_Client_ID = rs.getInt(5);
//						int AD_Org_ID = rs.getInt(6);
//						String sql1 = "UPDATE M_Cost SET CurrentCostPrice=" + currentcostprice +"  WHERE M_Product_ID=? AND C_AcctSchema_ID=? AND M_CostElement_ID=? AND AD_Client_ID=? AND AD_Org_ID=?";
//						 pstmt = null;			
//						try {
...
//						        
//						    }
//						String sql2 = "UPDATE M_CostDetail  SET CurrentCostPrice=" + currentcostprice +"  WHERE M_Product_ID=? AND C_AcctSchema_ID=? AND M_InventoryLine_ID=? AND AD_Client_ID=? AND AD_Org_ID=?";
//						 pstmt = null;			
//							try {
