...
import org.cyprusbrs.model.I_T_DistributionRunDetail;
import org.cyprusbrs.model.MBPartner;
import org.cyprusbrs.model.MDistributionRun;
import org.cyprusbrs.model.MDistributionRunDetail;
import org.cyprusbrs.model.MDistributionRunLine;
	...
	private boolean distributionOrders()
	{
		...
		{	
			...
			if (p_M_Warehouse_ID <= 0)
			{
				...
				if(m_source == null)
					throw new CyprusException("Do not exist Defautl Warehouse Source");
		...
		if(ws==null)
			throw new CyprusException("Warehouse Intransit do not found");
		...
		for (int i = 0; i < m_details.length; i++)
		{
			...
			if(m_target==null)
				throw new CyprusException("Do not exist Default Warehouse Target");
			
			m_locator_to = MLocator.getDefault(m_target); 

			if (m_locator == null || m_locator_to == null)
			{
				throw new CyprusException("Do not exist default Locator for Warehouses");
			}
