...
import org.cyprusbrs.model.M_Registration;
...
public class RegisterSystem extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		if (sys.getName().equals("?") || sys.getName().length()<2)
			throw new CyprusUserError("Set System Name in System Record");
		if (sys.getUserName().equals("?") || sys.getUserName().length()<2)
			throw new CyprusUserError("Set User Name (as in Web Store) in System Record");
		if (sys.getPassword().equals("?") || sys.getPassword().length()<2)
			throw new CyprusUserError("Set Password (as in Web Store) in System Record");
		//	Registration
		...
		if (reg.getC_Location_ID() > 0)
		{
			...
			if (loc.getCity() == null || loc.getCity().length() < 2)
				throw new CyprusUserError("No City in Address");
		}
		if (loc == null)
			throw new CyprusUserError("Please enter Address with City");
