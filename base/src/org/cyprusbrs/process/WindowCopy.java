...
public class WindowCopy extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		if (from.get_ID() == 0)
			throw new CyprusUserError("@NotFound@ (from->) @AD_Window_ID@");
		MWindow to = new MWindow (getCtx(), p_AD_WindowTo_ID, get_TrxName());
		if (to.get_ID() == 0)
			throw new CyprusUserError("@NotFound@ (to<-) @AD_Window_ID@");
			...
			if (newTab.save())
			{
				...
				for (int j = 0; j < oldFields.length; j++)
				{
					...
					if (newField.save())
						...
						throw new CyprusUserError("@Error@ @AD_Field_ID@");
				}
			}
			else
				throw new CyprusUserError("@Error@ @AD_Tab_ID@");
		}
