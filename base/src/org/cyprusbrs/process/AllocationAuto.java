...
import org.cyprusbrs.model.MPaySelectionCheck;
import org.cyprusbrs.model.MPaySelectionLine;
...
public class AllocationAuto extends SvrProcess
{
			...
			for (int i = 0; i < m_invoices.length; i++)
			{
				...
				if (payment.getC_Currency_ID() == invoice.getC_Currency_ID())
				{
					...
					if (difference.signum() == 0)
					{
						...
						{
							throw new CyprusSystemError("Cannot create Allocation");
						}
						processAllocation();
						count++;
	...
	private int allocateBPOldestFirst() throws Exception
	{
		...
		{
			throw new CyprusSystemError("Allocated Payments=" + allocatedPayments
