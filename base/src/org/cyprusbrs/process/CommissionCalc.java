...
import org.cyprusbrs.model.MCommission;
import org.cyprusbrs.model.MCommissionAmt;
import org.cyprusbrs.model.MCommissionDetail;
import org.cyprusbrs.model.MCommissionLine;
import org.cyprusbrs.model.MCommissionRun;
...
public class CommissionCalc extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		if (m_com.get_ID() == 0)
			throw new CyprusUserError ("No Commission");
		...
		if (!comRun.save())
			throw new CyprusSystemError ("Could not save Commission Run");
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			if (!comAmt.save())
				throw new CyprusSystemError ("Could not save Commission Amt");
			//
			...
			if (lines[i].isCommissionOrders())
			{
				...
				if (users == null || users.length == 0)
					throw new CyprusUserError ("Commission Business Partner has no Users/Contact");
				if (users.length == 1)
				{
