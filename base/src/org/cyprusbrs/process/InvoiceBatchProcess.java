...
import org.cyprusbrs.model.MInvoiceBatch;
import org.cyprusbrs.model.MInvoiceBatchLine;
...
public class InvoiceBatchProcess extends SvrProcess
{
	...
	protected String doIt () throws Exception
	{
		...
		if (p_C_InvoiceBatch_ID == 0)
			throw new CyprusUserError("C_InvoiceBatch_ID = 0");
		MInvoiceBatch batch = new MInvoiceBatch(getCtx(), p_C_InvoiceBatch_ID, get_TrxName());
		if (batch.get_ID() == 0)
			throw new CyprusUserError("@NotFound@: @C_InvoiceBatch_ID@ - " + p_C_InvoiceBatch_ID);
		if (batch.isProcessed())
			throw new CyprusUserError("@Processed@");
		//
		if (batch.getControlAmt().signum() != 0
			&& batch.getControlAmt().compareTo(batch.getDocumentAmt()) != 0)
			throw new CyprusUserError("@ControlAmt@ <> @DocumentAmt@");		
		//
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			if (m_invoice == null)
			{
				...
				if (!m_invoice.save())
					throw new CyprusUserError("Cannot save Invoice");
				//
			...
			{
				//	rollback
				throw new CyprusUserError("Line " + line.getLine() + " TaxIncluded inconsistent");
			}
			...
			{
				//	rollback
				throw new CyprusUserError("Cannot save Invoice Line");
			}
