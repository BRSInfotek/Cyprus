...
import org.cyprusbrs.model.MCost;
import org.cyprusbrs.model.MCostElement;
...
public class CostUpdate extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		if (p_M_PriceList_Version_ID == 0
			...
			throw new CyprusUserError ("@FillMandatory@  @M_PriceList_Version_ID@");
		
		//	Validate Source
		if (!isValid(p_SetFutureCostTo))
			throw new CyprusUserError ("@NotFound@ @M_CostElement_ID@ (Future) " + p_SetFutureCostTo);
		if (!isValid(p_SetStandardCostTo))
			throw new CyprusUserError ("@NotFound@ @M_CostElement_ID@ (Standard) " + p_SetStandardCostTo);

		//	Prepare
		MClient client = MClient.get(getCtx());
		m_ce = MCostElement.getMaterialCostElement(client, MAcctSchema.COSTINGMETHOD_StandardCosting);
		if (m_ce.get_ID() == 0)
			throw new CyprusUserError ("@NotFound@ @M_CostElement_ID@ (StdCost)");
	...
	private BigDecimal getCosts (MCost cost, String to) throws Exception
	{
		...
		if (to.equals(TO_AverageInvoice))
		{
			...
			if (ce == null)
				throw new CyprusSystemError("CostElement not found: " + TO_AverageInvoice);
		...
		else if (to.equals(TO_AveragePO))
		{
			...
			if (ce == null)
				throw new CyprusSystemError("CostElement not found: " + TO_AveragePO);
			MCost xCost = MCost.get(getCtx(), cost.getAD_Client_ID(), cost.getAD_Org_ID(), cost.getM_Product_ID(), cost.getM_CostType_ID(), cost.getC_AcctSchema_ID(), ce.getM_CostElement_ID(), cost.getM_AttributeSetInstance_ID());
			if (xCost != null)
				retValue = xCost.getCurrentCostPrice();
		}
		...
		else if (to.equals(TO_FiFo))
		{
			...
			if (ce == null)
				throw new CyprusSystemError("CostElement not found: " + TO_FiFo);
			MCost xCost = MCost.get(getCtx(), cost.getAD_Client_ID(), cost.getAD_Org_ID(), cost.getM_Product_ID(), cost.getM_CostType_ID(), cost.getC_AcctSchema_ID(), ce.getM_CostElement_ID(), cost.getM_AttributeSetInstance_ID());
			if (xCost != null)
				retValue = xCost.getCurrentCostPrice();
		}
		...
		else if (to.equals(TO_LiFo))
		{
			...
			if (ce == null)
				throw new CyprusSystemError("CostElement not found: " + TO_LiFo);
			MCost xCost = MCost.get(getCtx(), cost.getAD_Client_ID(), cost.getAD_Org_ID(), cost.getM_Product_ID(), cost.getM_CostType_ID(), cost.getC_AcctSchema_ID(), ce.getM_CostElement_ID(), cost.getM_AttributeSetInstance_ID());
			if (xCost != null)
				retValue = xCost.getCurrentCostPrice();
		}
