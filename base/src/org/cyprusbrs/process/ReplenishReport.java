...
import org.cyprusbrs.model.MRequisitionLine;
import org.cyprusbrs.model.MStorage;
import org.cyprusbrs.model.MWarehouse;
import org.cyprusbrs.model.X_T_Replenish;
...
public class ReplenishReport extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		if (p_ReplenishmentCreate != null && p_C_DocType_ID == 0)
			throw new CyprusUserError("@FillMandatory@ @C_DocType_ID@");
		...
		if (!dt.getDocBaseType().equals(p_ReplenishmentCreate))
			throw new CyprusSystemError("@C_DocType_ID@=" + dt.getName() + " <> " + p_ReplenishmentCreate);
		//
	...
	private void createDO() throws Exception
	{
		...
		for (X_T_Replenish replenish:replenishs)
		{
				...
				|| M_Warehouse_ID != replenish.getM_Warehouse_ID())
			{
				...
				if (C_BPartner_ID==0)
					throw new CyprusUserError(Msg.translate(getCtx(), "C_BPartner_ID")+ " @FillMandatory@ ");
				...
				if (order.getM_Warehouse_ID()==0)
					throw new CyprusUserError("Warehouse inTransit is @FillMandatory@ ");
			...
			throw new CyprusUserError(Msg.translate(getCtx(), "M_Locator_ID")+" @FillMandatory@ ");
