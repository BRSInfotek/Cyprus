...
import org.cyprusbrs.model.MAcctSchemaElement;
import org.cyprusbrs.model.MAcctSchemaGL;
...
public class AcctSchemaCopyAcct extends SvrProcess
{
	...
	protected String doIt () throws Exception
	{
		...
		if (p_SourceAcctSchema_ID == 0 || p_TargetAcctSchema_ID == 0)
			throw new CyprusSystemError("ID=0");
		
		if (p_SourceAcctSchema_ID == p_TargetAcctSchema_ID)
			throw new CyprusUserError("Must be different");
		
		MAcctSchema source = MAcctSchema.get(getCtx(), p_SourceAcctSchema_ID, null);
		if (source.get_ID() == 0)
			throw new CyprusSystemError("NotFound Source C_AcctSchema_ID=" + p_SourceAcctSchema_ID);
		MAcctSchema target = new MAcctSchema (getCtx(), p_TargetAcctSchema_ID, get_TrxName());
		if (target.get_ID() == 0)
			throw new CyprusSystemError("NotFound Target C_AcctSchema_ID=" + p_TargetAcctSchema_ID);
		
		//
		MAcctSchemaElement[] targetElements = target.getAcctSchemaElements();
		if (targetElements.length == 0)
			throw new CyprusUserError("NotFound Target C_AcctSchema_Element");
		...
		if (sourceAcctElement == null)
			throw new CyprusUserError("NotFound Source AC C_AcctSchema_Element");
		...
		if (targetAcctElement == null)
			throw new CyprusUserError("NotFound Target AC C_AcctSchema_Element");
		if (sourceAcctElement.getC_Element_ID() != targetAcctElement.getC_Element_ID())
			throw new CyprusUserError("@C_Element_ID@ different");
	...
	private void copyGL (MAcctSchema targetAS) throws Exception
	{
		...
		if (!target.save())
			throw new CyprusSystemError("Could not Save GL");
	}	//	copyGL
	...
	private void copyDefault(MAcctSchema targetAS) throws Exception
	{
		...
		if (!target.save())
			throw new CyprusSystemError("Could not Save Default");
	}	//	copyDefault
