...
import org.cyprus.common.FieldType;
import org.cyprusbrs.model.MRole;
import org.cyprusbrs.model.PO;
import org.cyprusbrs.model.X_AD_PrintFormatItem;
...
import org.cyprusbrs.util.Evaluator;
...
public class MPrintFormatItem extends X_AD_PrintFormatItem
{
	...
	public static MPrintFormatItem createFromBViewColumn(MPrintFormat format, int AD_Column_ID, int AD_BView_ID, int seqNo) {
	    MPrintFormatItem pfi = new MPrintFormatItem(format.getCtx(), 0, null);
	    pfi.setAD_PrintFormat_ID(format.getAD_PrintFormat_ID());
	    pfi.setClientOrg((PO)format);
	    pfi.setAD_Column_ID(AD_Column_ID);
	    pfi.setPrintFormatType(PRINTFORMATTYPE_Field);
	    String sql = "SELECT c.ColumnName,bvf.Name,bvf.Name, c.AD_Reference_ID,c.IsKey,bvf.SeqNo ,bvf.IsPrinted FROM AD_Column c, AD_BView_Source bvs, AD_BView_Source_Mapping bvsm, AD_BView_Field bvf WHERE c.AD_Column_ID=? AND bvsm.AD_BView_Source_Field_ID = c.AD_Column_ID AND bvsm.AD_BView_Source_ID = bvs.AD_BView_Source_ID and bvs.IsDefault = 'Y' AND bvs.AD_BView_ID = ? AND bvsm.AD_BView_Field_ID = bvf.AD_BView_Field_ID";
	    Language language = format.getLanguage();
	    boolean trl = (!Env.isMultiLingualDocument(format.getCtx()) && !language.isBaseLanguage());
	    trl = false;
	    if (trl)
	    ...
	    try {
	      ...
	      cPreparedStatement.setInt(1, AD_Column_ID);
	      cPreparedStatement.setInt(2, AD_BView_ID);
	      if (trl)
	        cPreparedStatement.setString(3, language.getAD_Language()); 
