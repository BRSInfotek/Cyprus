...
public class ImmediateBankTransfer extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		if (p_To_C_BankAccount_ID == p_From_C_BankAccount_ID)
			throw new CyprusUserError ("Banks From and To must be different");

		// Banks and CashBook must have same currency
		if (!isSameCurrency())
			throw new CyprusUserError ("Banks and CashBook must have same currency");		

		if (p_Amount.compareTo(new BigDecimal(0)) == 0)
			throw new CyprusUserError ("Amount required");
