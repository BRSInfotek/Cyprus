...
public class GenerateModelJPA
{
	...
	private String createHeader (int AD_Table_ID, StringBuffer sb, StringBuffer mandatory, String packageName)
	{
		...
		if (!packageName.equals("org.cyprus.model"))
			start.append("import org.cyprus.model.*;");	
		start.append("import java.util.*;"
			+ "import java.sql.*;"
			+ "import java.math.*;"
                        + "import javax.persistence.*;"
			+ "import org.cyprusbrs.util.*;"
                        + "import org.cyprus.util.*;"                        
			//	Class
			+ "/** Generated Model for ").append(tableName).append("\n"
			+ " *  @author Victor Perez (generated) \n"
			+ " *  @version ").append(Cyprus.MAIN_VERSION).append(" - ").append(s_run).append(" */\n"
                        + " @Entity"
...
//			+ "}"	//	Constructor End
	...
	//	CLogMgt.setLevel(Level.ALL);
		...
		log.info("----------------------------------");
		//	first parameter
		String directory = "/app/cyprus/cyprus_branch_3.1.1/dbPort/src/org/cyprus/model/";
		if (args.length > 0)
			directory = args[0];
		...
		//	second parameter
		String packageName = "cyprus.model";
		if (args.length > 1)
			packageName = args[1]; 
