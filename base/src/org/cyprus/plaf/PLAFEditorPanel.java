...
public class PLAFEditorPanel extends CPanel {
	...
	private JList lookList = new JList(CyprusPLAF.getPLAFs());
	...
	private void setupUI() {
		...
		label.setForeground(CyprusPLAF.getPrimary1());
		...
		scrollPane.setBorder(BorderFactory.createLineBorder(CyprusPLAF.getSecondary1(), 1));
	...
	protected void themeSelectionChanged(ListSelectionEvent e) {
		...
		CyprusPLAF.setPLAF(laf, theme, false);
	...
	protected void lookAndFeelSelectionChanged(ListSelectionEvent e) {
		...
		LookAndFeel currentLaf = UIManager.getLookAndFeel();
		MetalTheme currentTheme = MetalLookAndFeel.getCurrentTheme();
		CyprusPLAF.setPLAF(laf, null, false);
		previewPanel.refresh(currentLaf,currentTheme);
		SwingUtilities.updateComponentTreeUI(previewPanel);
		updatePreviewComponents();
		setLFSelection();
	...
	private void setupPreview() {
		...
		previewPanel.setBorder(BorderFactory.createLineBorder(CyprusPLAF.getSecondary1(), 1));
		...
		jtree.setBorder(BorderFactory.createLineBorder(CyprusPLAF.getSecondary1(), 1));
	...
	private void setLFSelection()
	{
		...
		themeList.setModel(new DefaultComboBoxModel(CyprusPLAF.getThemes()));
		if (metal)
		{
			theme = null;
		...
		if (capture) {
			...
			if (laf != null) {
				...
				if (laf instanceof MetalLookAndFeel)
					CyprusPLAF.setCurrentMetalTheme((MetalLookAndFeel)laf, theme);
				try {
