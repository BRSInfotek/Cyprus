...
import org.cyprusbrs.plaf.CompiereLookAndFeel;
import org.cyprusbrs.plaf.CompiereThemeBlueMetal;
import org.cyprusbrs.plaf.CompiereThemeIce;
import org.cyprusbrs.swing.CButton;
import org.cyprusbrs.swing.ColorBlind;
import org.cyprusbrs.swing.ExtendedTheme;
 ...
 *  @version 2006-11-27
 */
public final class CyprusPLAF
{
	/**	Logger			*/
	private static Logger log = Logger.getLogger(CyprusPLAF.class.getName());
	...
	static
	{
		...
		try
		{
			...
			vp = new ValueNamePair("org.cyprus.plaf.AdempiereLookAndFeel", org.cyprus.plaf.CyprusLookAndFeel.NAME);
			plafList.add(vp);
			s_vp_adempiereTheme = new ValueNamePair("org.cyprus.plaf.AdempiereTheme", CyprusThemeInnova.NAME);
			plasticThemes.add (s_vp_adempiereTheme);
			
			List<?> installedThemes = CyprusLookAndFeel.getInstalledThemes();
		...
		//e-evolution vpj-cd 19102006
		vp = new ValueNamePair("org.cyprusbrs.plaf.CompiereLookAndFeel", CompiereLookAndFeel.NAME);
		plafList.add (vp);
		
		//Metal
		s_vp_compiereTheme = new ValueNamePair("org.cyprusbrs.plaf.CompiereThemeBlueMetal", CompiereThemeBlueMetal.NAME);
		metalThemes.add (s_vp_compiereTheme);
		metalThemes.add (new ValueNamePair("org.cyprusbrs.plaf.CompiereThemeIce", CompiereThemeIce.NAME));
		...
		metalThemes.add (new ValueNamePair("org.cyprusbrs.plaf.AquaTheme", "Aqua"));
		metalThemes.add (new ValueNamePair("org.cyprusbrs.plaf.CharcoalTheme", "Charcoal"));
		metalThemes.add (new ValueNamePair("org.cyprusbrs.plaf.ContrastTheme", "Contrast"));
		metalThemes.add (new ValueNamePair("org.cyprusbrs.plaf.EmeraldTheme", "Emerald"));
		metalThemes.add (new ValueNamePair("org.cyprusbrs.plaf.RubyTheme", "Ruby"));
		...
		for (int i = 0; i < lfInfo.length; i++)
		{
			...
			plafList.add(vp);
			if (lfInfo[i].getName().equals(org.cyprus.plaf.CyprusLookAndFeel.NAME))
			{
				s_defaultPLAF = vp;
	...
	public static ValueNamePair[] getThemes ()
	{
		...
		if ( l instanceof CyprusLookAndFeel)
	...
	public static void setPLAF (ValueNamePair plaf, ValueNamePair theme, boolean updateIni)
	{
		...
		boolean adempiere = CyprusLookAndFeel.class.isAssignableFrom(lafClass);
		boolean cyprusbrs = CompiereLookAndFeel.class.isAssignableFrom(lafClass);
		if (theme == null && metal)
		{
			if (cyprusbrs)
		...
		if (theme != null && metal && theme.getValue().length() > 0)
		{
			try
			{
				...
				if (cyprusbrs)
					...
					CyprusLookAndFeel.setCurrentTheme((PlasticTheme)t);
				else 
	...
	{
		return UIManager.getLookAndFeel() instanceof CyprusLookAndFeel;
	}   //  isActive

	/*************************************************************************/

	static ResourceBundle   s_res = ResourceBundle.getBundle("org.cyprusbrs.plaf.PlafRes");
	...
	public static CButton getOKButton()
	{
		...
		b.setIcon(new ImageIcon(CyprusPLAF.class.getResource("icons/Ok24.gif")));
	...
	{
		CButton b = new CButton();
		b.setIcon(new ImageIcon(CyprusPLAF.class.getResource("icons/Cancel24.gif")));
		b.setMargin(new Insets(0,10,0,10));
	...
	public static void setCurrentMetalTheme(MetalLookAndFeel laf, MetalTheme theme)
	{
		...
		else if (laf instanceof CyprusLookAndFeel && theme instanceof PlasticTheme)
			CyprusLookAndFeel.setCurrentTheme((PlasticTheme)theme);
		else 
