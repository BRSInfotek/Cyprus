...
import org.cyprusbrs.swing.CToggleButton;
import org.cyprusbrs.swing.ColorBlind;
import org.cyprusbrs.util.Ini;
import org.cyprusbrs.util.MiniBrowser;
	...
	implements ActionListener
{
	...
	private void init (boolean showExample)
	{
		try
		{
			...
			CyprusPLAF.showCenterScreen(this);
		}
		catch(Exception e)
		{
	...
	private CButton bOK = CyprusPLAF.getOKButton();
	private CButton bCancel = CyprusPLAF.getCancelButton();
	private CButton bHelp = new CButton();	/** @todo Help Button */
	private GridBagLayout northLayout = new GridBagLayout();
	private CLabel lfLabel = new CLabel();
	private CComboBox lfField = new CComboBox(CyprusPLAF.getPLAFs());
	private CLabel themeLabel = new CLabel();
	private CComboBox themeField = new CComboBox(CyprusPLAF.getThemes());
	...
	private void setLFSelection()
	{
		...
		for (int i = 0; i < CyprusPLAF.getPLAFs().length; i++)
		{
			ValueNamePair vp = CyprusPLAF.getPLAFs()[i];
		...
		themeField.setModel(new DefaultComboBoxModel(CyprusPLAF.getThemes()));
		if (metal)
		{
			theme = null;
			...
			if (metalTheme != null)
			{
				...
				for (int i = 0; i < CyprusPLAF.getThemes().length; i++)
				{
					ValueNamePair vp = CyprusPLAF.getThemes()[i];
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if (e.getSource() == lfField && !m_setting)
		{
			...
			//  set new theme
			CyprusPLAF.setPLAF((ValueNamePair)lfField.getSelectedItem(), null, true);
			CyprusPLAF.updateUI(this);
			setLFSelection();
		...
		else if (e.getSource() == themeField && !m_setting)
		{
			...
			CyprusPLAF.setPLAF((ValueNamePair)lfField.getSelectedItem(), (ValueNamePair)themeField.getSelectedItem(),true);
			CyprusPLAF.updateUI(this);
		}
