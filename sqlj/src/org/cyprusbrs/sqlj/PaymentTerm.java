...
public class PaymentTerm
{
		...
		throws SQLException
	{
		...
		PreparedStatement pstmt = Cyprus.prepareStatement(sql);
		...
		if (rs.next())
		{
			...
			else
			{
				...
				DueDate = Cyprus.addDays(p_DocDate, NetDays);
			}
		}
		rs.close();
		pstmt.close();
		...
		if (DueDate == null)
			return 0;
		return Cyprus.getDaysBetween(DueDate, PayDate);
	}	//	dueDays
		...
		throws SQLException
	{
		...
		if (p_C_PaymentTerm_ID == 0 || p_DocDate == null)
			return null;
		//	Due Date
		Timestamp DueDate = Cyprus.trunc(p_DocDate);
		...
		if (rs.next())
		{
			...
			else
			{
				int NetDays = rs.getInt("NetDays");
				if (NetDays != 0)
					DueDate = Cyprus.addDays(DueDate, NetDays);
			}
		}
		rs.close();
		pstmt.close();
		...
		throws SQLException
	{
		...
		if (rs.next())
		{
			...
			//
			Timestamp Discount1Date = Cyprus.addDays(p_DocDate, DiscountDays + GraceDays);
			Timestamp Discount2Date = Cyprus.addDays(p_DocDate, DiscountDays2 + GraceDays);
			//	Next Business Day
			if (IsNextBusinessDay)
			{
				Discount1Date = Cyprus.nextBusinessDay(Discount1Date);
				Discount2Date = Cyprus.nextBusinessDay(Discount2Date);
			}
			...
			//	Discount 2
				...
				discount = Cyprus.ZERO;
			//	Divide
			if (discount.signum() != 0)
			{
				discount = discount.divide(Cyprus.HUNDRED, 6, BigDecimal.ROUND_HALF_UP);
