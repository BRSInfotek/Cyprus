...
public class Invoice
{
		...
		**/
		...
		if (p_C_InvoicePaySchedule_ID > 0)	//	if not valid = lists invoice amount
		{
			...
			sql = "SELECT C_InvoicePaySchedule_ID, DueAmt "
				...
				+ " AND IsValid='Y' "
				+ "ORDER BY DueDate";
			pstmt = Cyprus.prepareStatement(sql);
			pstmt.setInt(1, p_C_Invoice_ID);
	...
	{
		//	Calculate Allocated Amount
		BigDecimal paidAmt = Cyprus.ZERO;
		...
		throws SQLException
	{
		...
		{
			BigDecimal discount = Cyprus.ZERO;
