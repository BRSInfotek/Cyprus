...
import org.cyprusbrs.model.MAcctProcessor;
import org.cyprusbrs.model.MAlertProcessor;
import org.cyprusbrs.model.MIMPProcessor;
import org.cyprusbrs.model.MLdapProcessor;
import org.cyprusbrs.model.MRequestProcessor;
import org.cyprusbrs.model.MScheduler;
 ...
 */
public class CyprusServerMgr
{
	/**
	 * 	Get Adempiere Server Manager
	 *	@return mgr
	 */
	public static CyprusServerMgr get()
	{
		if (m_serverMgr == null)
		{
			//	for faster subsequent calls
			m_serverMgr = new CyprusServerMgr();
	...
	/**	Singleton					*/
	private static	CyprusServerMgr	m_serverMgr = null;
	/**	Logger			*/
	...
	/**	The Servers				*/
	private ArrayList<CyprusServer>	m_servers = new ArrayList<CyprusServer>();
	/** Context					*/
	...
	private boolean startServers()
	{
		...
		for (int i = 0; i < acctModels.length; i++)
		{
			...
			CyprusServer server = CyprusServer.create(pModel);
			server.start();
		...
		for (int i = 0; i < ldapModels.length; i++)
		{
			...
			CyprusServer server = CyprusServer.create(lp);
			server.start();
	...
	{
		log.info ("");
		CyprusServer[] servers = getInActive();
		for (int i = 0; i < servers.length; i++)
		{
			...
			try
			{
				...
				{
					//	replace
					server = CyprusServer.create (server.getModel());
					if (server == null)
						m_servers.remove(i);
					else
	...
	{
		CyprusServer server = getServer(serverID);
		if (server == null)
			return false;
		if (server.isAlive())
			return true;
	...
	{
		log.info ("");
		CyprusServer[] servers = getActive();
		//	Interrupt
		for (int i = 0; i < servers.length; i++)
		{
			CyprusServer server = servers[i];
	 ...
	 */
	protected CyprusServer[] getActive()
	{
		ArrayList<CyprusServer> list = new ArrayList<CyprusServer>();
		for (int i = 0; i < m_servers.size(); i++)
		{
			CyprusServer server = (CyprusServer)m_servers.get(i);
			if (server != null && server.isAlive() && !server.isInterrupted())
				list.add (server);
		}
		CyprusServer[] retValue = new CyprusServer[list.size ()];
	 ...
	 */
	protected CyprusServer[] getInActive()
	{
		ArrayList<CyprusServer> list = new ArrayList<CyprusServer>();
		for (int i = 0; i < m_servers.size(); i++)
		{
	 ...
	 */
	public CyprusServer[] getAll()
	{
		CyprusServer[] retValue = new CyprusServer[m_servers.size()];
	 ...
	 */
	public CyprusServer getServer (String serverID)
	{
		if (serverID == null)
			return null;
		for (int i = 0; i < m_servers.size(); i++)
	...
	{
		StringBuffer sb = new StringBuffer ("CyprusServerMgr[");
