...
import org.cyprus.server.rpl.IImportProcessor;
import org.cyprusbrs.model.CyprusProcessor;
import org.cyprusbrs.model.MClient;
import org.cyprusbrs.model.MIMPProcessor;
import org.cyprusbrs.model.MIMPProcessorLog;
import org.cyprusbrs.model.X_IMP_Processor_Type;
 ...
 *
 */
public class ReplicationProcessor extends CyprusServer {
	...
	protected ReplicationProcessor(CyprusProcessor model, int initialNap) {
	super (model, initialNap);
	mImportProcessor = (MIMPProcessor)model;
	m_client = MClient.get(mImportProcessor.getCtx(), mImportProcessor.getAD_Client_ID());
}
	protected ReplicationProcessor(CyprusProcessor model) {
		super (model, 30);
		mImportProcessor =(MIMPProcessor)model;
		m_client = MClient.get(mImportProcessor.getCtx(), mImportProcessor.getAD_Client_ID());
	}
