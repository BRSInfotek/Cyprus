...
import org.cyprusbrs.db.CyprusDatabase;
import org.cyprusbrs.model.CyprusProcessorLog;
import org.cyprusbrs.model.MClient;
import org.cyprusbrs.model.MStore;
import org.cyprusbrs.model.MSystem;
import org.cyprusbrs.server.CyprusServer;
import org.cyprusbrs.server.CyprusServerMgr;
import org.cyprusbrs.util.CLogFile;
import org.cyprusbrs.util.CLogMgt;
import org.cyprusbrs.util.CLogger;
import org.cyprusbrs.util.CMemoryUsage;
...
/**
 *	Cyprus Server Monitor
 *	
 *  @author Jorg Janke
 *  @version $Id: CyprusMonitor.java,v 1.3 2006/07/30 00:53:33 jjanke Exp $
 */
public class CyprusMonitor extends HttpServlet
{
	/**
	 * 
	 */
	...
	/**	Logger				*/
	private static CLogger	log = CLogger.getCLogger(CyprusMonitor.class);
	/**	The Server			*/
	private CyprusServerMgr	m_serverMgr = null;
	/** Message				*/
		...
		throws ServletException, IOException
	{
		...
		CyprusServer server = m_serverMgr.getServer(serverID);
		if (server == null)
		{
			m_message = new p();
		...
		WebDoc doc = WebDoc.create ("Cyprus Server Monitor Log");
		//	Body
		body b = doc.getBody();
		//
		p para = new p();
		a link = new a ("cyprusMonitor#" + serverID, "Return");
	...
	//	line.addElement(new th().addElement("Description"));
		...
		CyprusProcessorLog[] logs = server.getLogs();
		for (int i = 0; i < logs.length; i++)
		{
			CyprusProcessorLog pLog = logs[i];
			line = new tr();
	...
	private void createSummaryPage (HttpServletRequest request, HttpServletResponse response)
		throws ServletException, IOException
	{
		WebDoc doc = WebDoc.create ("Cyprus Server Monitor");
	//	log.info("ServletConfig=" + getServletConfig());
	//	CyprusServerGroup.get().dump();
		...
		//
		tr line = new tr();
		line.addElement(new th().addElement(Cyprus.getName()));
		line.addElement(new td().addElement(Cyprus.getVersion()));
		table.addElement(line);
		line = new tr();
		line.addElement(new th().addElement(Cyprus.getImplementationVendor()));
		line.addElement(new td().addElement(Cyprus.getImplementationVersion()));
		table.addElement(line);
		line = new tr();
		...
		//
		p para = new p();
		a link = new a ("cyprusMonitor?Action=Start_All", "Start All");
		para.addElement(link);
		para.addElement(" - ");
		link = new a ("cyprusMonitor?Action=Stop_All", "Stop All");
		para.addElement(link);
		para.addElement(" - ");
		link = new a ("cyprusMonitor", "Refresh");
		para.addElement(link);
		bb.addElement(para);
		...
		bb.addElement(new hr());
		para = new p();
		CyprusServer[] servers = m_serverMgr.getAll();		
		for (int i = 0; i < servers.length; i++)
		{
			if (i > 0)
				para.addElement(new br());
			CyprusServer server = servers[i];
		...
		for (int i = 0; i < servers.length; i++)
		{
			...
			if (server.isAlive())
			{
				...
				if (server.isInterrupted())
					msg += " (Interrupted)";
				link = new a ("cyprusMonitor?Action=Stop_" + server.getServerID(), msg);
			...
			else
			{
				...
				link = new a ("cyprusMonitor?Action=Start_" + server.getServerID(), msg);
				line.addElement(new td().addElement(link));
			}
			table.addElement(line);
			//
			...
			td.addElement(" - ");
			link = new a ("cyprusMonitor?RunNow=" + server.getServerID(), "(Run Now)");
			td.addElement(link);
			line.addElement(td);
			table.addElement(line);
			//
			...
			bb.addElement(link);
			bb.addElement(" - ");
			link = new a ("cyprusMonitor?Log=" + server.getServerID(), "Log");
			bb.addElement(link);
			bb.addElement(" - ");
			link = new a ("cyprusMonitor", "Refresh");
			bb.addElement(link);
	...
	private void createLogMgtPage (body bb)
	{
		...
		line.addElement(new td().addElement(Ini.getCyprusHome()));
		table.addElement(line);
		//	OS + Name
		line = new tr();
		...
		CyprusDatabase db = cc.getDatabase();
...
//		line.addElement(new td().addElement(system.getDBInstance()));
		...
		line.addElement(new td().addElement(new a ("cyprusMonitor?CacheReset=Yes", "Reset Cache")));
		table.addElement(line);
		
		//	Trace Level
		line = new tr();
		line.addElement(new th().addElement(new label("TraceLevel").addElement("Trace Log Level")));
		form myForm = new form("cyprusMonitor", form.METHOD_POST, form.ENC_DEFAULT);
		//	LogLevel Selection
		...
		line.addElement(new td().addElement(new a ("cyprusMonitor?Trace=" + fileHandler.getFileName(), "Current")));
		table.addElement(line);
		//
		line = new tr();
		line.addElement(new td().addElement(new a ("cyprusMonitor?Trace=ROTATE", "Rotate Trace Log")));
		line.addElement(new td().addElement(new a ("cyprusMonitor?Trace=DELETE", "Delete all Trace Logs")));
		table.addElement(line);
		//
		bb.addElement(table);
		...
		if (logDir != null && logDir.isDirectory())
		{
			...
			for (int i = 0; i < logs.length; i++) 
			{
				...
				if (i != 0)
					p.addElement(" - ");
				String fileName = logs[i].getAbsolutePath();
				a link = new a ("cyprusMonitor?Trace=" + fileName, fileName);
				p.addElement(link);
		...
		for (int i = 0; i < clients.length; i++)
		{
			...
			if (i > 0)
				p.addElement(" - ");
			p.addElement(new a("cyprusMonitor?EMail=" + client.getAD_Client_ID(), client.getName()));
		}
		if (clients.length == 0)
			p.addElement("&nbsp;");
	...
	public void init (ServletConfig config) throws ServletException
	{
		...
		log.info ("");
		m_serverMgr = CyprusServerMgr.get();
	}	//	init
