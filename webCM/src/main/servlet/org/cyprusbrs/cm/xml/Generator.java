...
import org.cyprusbrs.cm.HttpServletCM;
import org.cyprusbrs.cm.cache.Chat;
import org.cyprusbrs.cm.cache.ContainerElement;
import org.cyprusbrs.cm.cache.ContainerTree;
import org.cyprusbrs.cm.utils.Counter;
import org.cyprusbrs.cm.utils.RequestAnalyzer;
import org.cyprusbrs.model.MAd;
import org.cyprusbrs.model.MBPBankAccount;
...
import org.cyprusbrs.model.MNewsChannel;
import org.cyprusbrs.model.MTable;
import org.cyprusbrs.model.MTemplate;
import org.cyprusbrs.model.MUser;
import org.cyprusbrs.model.PO;
import org.cyprusbrs.model.X_CM_Container_Element;
import org.cyprusbrs.model.X_CM_NewsChannel;
import org.cyprusbrs.model.X_CM_TemplateTable;
import org.cyprusbrs.util.DB;
import org.cyprusbrs.util.Language;
import org.cyprusbrs.util.WebInfo;
...
public class Generator
{
		...
		HttpServletRequest httpRequest, RequestAnalyzer tempRequest, StringBuffer xmlAppend)
	{
		...
		xmlCode.append ("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n").append (
			"<!-- ").append (org.cyprusbrs.Cyprus.getSummaryAscii ()).append (
			" -->\n").append ("<webCM>\n").append ("<build>").append (
	...
	private void generateSystemHeader (HttpServletCM thisServlet)
	{
		...
		xmlCode.append ("<mainversion>" + Cyprus.MAIN_VERSION
			+ "</mainversion>\n");
		xmlCode.append ("<dateversion>" + Cyprus.DATE_VERSION
			+ "</dateversion>\n");
		xmlCode.append ("<dbversion>" + Cyprus.DB_VERSION + "</dbversion>\n");
		xmlCode.append ("</adempiere>\n");
	...
	private void generateCommunity(org.cyprusbrs.cm.cache.XML thisXML) 
	{
	 ...
	 * @param ctx
	 */
	private void generateRequest(HttpServletRequest httpRequest, org.cyprusbrs.cm.cache.XML thisXML, Properties ctx) 
	{
		HttpSession thisSession = httpRequest.getSession (false);
		WebInfo wi = null;
		if (thisSession!=null) {
	...
	public String replaceSessionElements(WebInfo wi, String whereClause) 
	{
		if (wi!=null && whereClause!=null && whereClause.indexOf ("@")>=0) {
			if (whereClause.indexOf ("@C_BPartner_ID@")>0)
				whereClause = org.cyprusbrs.util.Util.replace(whereClause,"@C_BPartner_ID@", "" + wi.getC_BPartner_ID ());
		}
		return whereClause;
	}
				...
				if (vals != null)
				{
					for (int k = 0; k < vals.length; k++)
					{
						xmlCode.append ("      <name>"
							+ nameArray[i]
							+ "</name><value>"
							+ org.cyprusbrs.util.Util.replace (
								org.cyprusbrs.util.Util.replace (vals[k],
									"'", "&#39;"), "&", "&amp;") + "</value>\n");
					}
				}
				else
