...
import org.cyprus.plaf.PLAFEditorPanel;
	...
	implements ActionListener, ListSelectionListener
{
	...
	void jbInit() throws Exception
	{
		...
		infoList.setBackground(CyprusPLAF.getFieldBackground_Inactive());
...
//		tabPane.add(errorPane,  Msg.getMsg(Env.getCtx(), "Errors"));
	...
	private void cmd_save()
	{
		...
		if ( laf != null ) {
			...
			{
				//laf changed
				CyprusPLAF.setPLAF(laf, theme, true);
				AEnv.updateUI();
			}
			else
			{
				if (UIManager.getLookAndFeel() instanceof MetalLookAndFeel)
				{
					...
					if (sTheme != null && sTheme.length() > 0 && !sTheme.equals(themeClass))
					{
						...
						CyprusPLAF.setPLAF(plaf, theme, true);
						AEnv.updateUI();
					}
				}
			}
