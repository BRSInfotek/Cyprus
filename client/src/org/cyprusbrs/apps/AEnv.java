...
import org.cyprusbrs.grid.ed.Calculator;
import org.cyprusbrs.interfaces.Server;
import org.cyprusbrs.model.GridWindowVO;
import org.cyprusbrs.model.MMenu;
...
import org.cyprusbrs.swing.CMenuItem;
...
public final class AEnv
{
	...
	//	}
		...
		{
			org.cyprusbrs.apps.search.Info.showProduct (Env.getFrame(c), WindowNo);
		}
		else if (actionCommand.equals("InfoBPartner") && AEnv.canAccessInfo("BPARTNER"))
		{
			org.cyprusbrs.apps.search.Info.showBPartner (Env.getFrame(c), WindowNo);
		}
		else if (actionCommand.equals("InfoAsset") && AEnv.canAccessInfo("ASSET"))
		{
			org.cyprusbrs.apps.search.Info.showAsset (Env.getFrame(c), WindowNo);
		}
		else if (actionCommand.equals("InfoAccount") && 
				  MRole.getDefault().isShowAcct() &&
				  AEnv.canAccessInfo("ACCOUNT"))
		{
			new org.cyprusbrs.acct.AcctViewer();
		}
		else if (actionCommand.equals("InfoSchedule") && AEnv.canAccessInfo("SCHEDULE"))
		{
			new org.cyprusbrs.apps.search.InfoSchedule (Env.getFrame(c), null, false);
		}
		//FR [ 1966328 ] 
		...
		{
			org.cyprusbrs.apps.search.Info.showOrder (Env.getFrame(c), WindowNo, "");
		}
		else if (actionCommand.equals("InfoInvoice") && AEnv.canAccessInfo("INVOICE"))
		{
			org.cyprusbrs.apps.search.Info.showInvoice (Env.getFrame(c), WindowNo, "");
		}
		else if (actionCommand.equals("InfoInOut") && AEnv.canAccessInfo("INOUT"))
		{
			org.cyprusbrs.apps.search.Info.showInOut (Env.getFrame(c), WindowNo, "");
		}
		else if (actionCommand.equals("InfoPayment") && AEnv.canAccessInfo("PAYMENT"))
		{
			org.cyprusbrs.apps.search.Info.showPayment (Env.getFrame(c), WindowNo, "");
		}
		else if (actionCommand.equals("InfoCashLine") && AEnv.canAccessInfo("CASHJOURNAL"))
		{
			org.cyprusbrs.apps.search.Info.showCashLine (Env.getFrame(c), WindowNo, "");
		}
		else if (actionCommand.equals("InfoAssignment") && AEnv.canAccessInfo("RESOURCE"))
		{
			org.cyprusbrs.apps.search.Info.showAssignment (Env.getFrame(c), WindowNo, "");
		}
		...
		{
			Calculator calc = new org.cyprusbrs.grid.ed.Calculator(Env.getFrame(c));
			calc.setDisposeOnEqual(false);
			AEnv.showCenterScreen (calc);
		}
		else if (actionCommand.equals("Calendar"))
		{
			AEnv.showCenterScreen (new org.cyprusbrs.grid.ed.Calendar(Env.getFrame(c)));
		}
		else if (actionCommand.equals("Editor"))
		{
			AEnv.showCenterScreen (new org.cyprusbrs.grid.ed.Editor(Env.getFrame(c)));
		}
		...
		{
			Env.startBrowser(org.cyprusbrs.Cyprus.getOnlineHelpURL());
		}
