...
import org.cyprus.pos.grid.BrowserSearch;
import org.cyprus.pos.grid.IBrowserTable;
import org.cyprus.pos.model.MBrowse;
import org.cyprus.pos.model.MViewDefinition;
import org.cyprus.pos.model.ProcessController;
import org.cyprus.pos.spin.util.ASPUtil;
...
import org.cyprusbrs.apps.Waiting;
...
public class VBrowser extends Browser implements ActionListener, ListSelectionListener, ASyncProcess {
	...
	private boolean initBrowser() {
		//	
		initBrowserTable((IBrowserTable) detail);
		//	
	...
	private void cmd_zoom() {
		...
		MQuery query = getMQuery((IBrowserTable) detail);
		if(query != null)
	...
	private void cmd_deleteSelection(){
		...
		if (ADialog.ask(getWindowNo(), m_frame.getContainer(), "DeleteSelection")) {	
			int records = deleteSelection((IBrowserTable) detail);
	...
	private void cmd_updateSelection(){
		...
		if (ADialog.ask(getWindowNo(), m_frame.getContainer(), "UpdateSelection")) {	
			int records = updateSelection((IBrowserTable) detail);
	...
	private void cmd_Process() {
		...
		m_worker = null;
		saveResultSelection((IBrowserTable) detail);
		saveSelection((IBrowserTable) detail);
		//	Is Process ok
		boolean isOk = false;
	...
	private void cmd_Export() {
		m_frame.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
		try 
		{	File file = exportXLS((IBrowserTable) detail);
	...
	class Worker extends Thread {
		...
		public void run() {
			...
			if(isAllSelected) {
				selectedRows((IBrowserTable) detail);
			}
			//	Set Collapsed
			if(row > 0)
