...
public final class VCellRenderer extends DefaultTableCellRenderer
{
	...
	//		+ ", Row=" + row + ", Col=" + col);
		...
		Color bg = CyprusPLAF.getFieldBackground_Normal();
		Color fg = CyprusPLAF.getTextColor_Normal();
		//  Inactive Background
		boolean ro = !table.isCellEditable(row, col); 
		if (ro)
		{
			bg = CyprusPLAF.getFieldBackground_Inactive();
		...
		int cCode = 0;
		//  Grid
		if (table instanceof org.cyprusbrs.grid.VTable)
			cCode = ((org.cyprusbrs.grid.VTable)table).getColorCode (row);
		//  MiniGrid
		else if (table instanceof org.cyprusbrs.minigrid.MiniTable)
			cCode = ((org.cyprusbrs.minigrid.MiniTable)table).getColorCode (row);
		//
		if (cCode == 0)
			;											//	Black
		else if (cCode < 0)
			fg = CyprusPLAF.getTextColor_Issue();		//	Red
		else
			fg = CyprusPLAF.getTextColor_OK();		//	Blue
		...
		//	Windows is white on blue
			...
			if (hasFocus)
				bg = org.cyprus.apps.graph.GraphUtil.brighter(bg, .9);
		}
